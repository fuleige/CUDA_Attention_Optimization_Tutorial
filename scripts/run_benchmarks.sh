...
echo "kernel,dtype,m,n,k,avg_ms,tflops_est,max_abs_err,max_rel_err,pass" > "${GEMM_OUT}"
echo "kernel,dtype,batch,heads,kv_heads,seq_q,seq_kv,head_dim,avg_ms,tflops_est,bandwidth_est,max_abs_err,max_rel_err,pass" > "${ATTN_FWD_OUT}"
echo "kernel,dtype,avg_ms,max_abs_err,max_rel_err,pass" > "${ATTN_BWD_OUT}"

"${ROOT_DIR}/bin/gemm_runner" --kernel naive --dtype fp32 --m 256 --n 256 --k 256 --warmup 5 --iters 20 --check true --csv true >> "${GEMM_OUT}"
...
"${ROOT_DIR}/bin/attention_runner" --kernel basic_fwd --dtype fp32 --batch 1 --heads 8 --kv-heads 8 --seq-q 128 --seq-kv 128 --head-dim 64 --causal true --warmup 3 --iters 10 --check true --csv true >> "${ATTN_FWD_OUT}"
...
"${ROOT_DIR}/bin/attention_runner" --kernel basic_bwd --dtype fp32 --batch 1 --heads 4 --kv-heads 4 --seq-q 64 --seq-kv 64 --head-dim 32 --causal true --warmup 2 --iters 5 --check true --csv true >> "${ATTN_BWD_OUT}"
"${ROOT_DIR}/bin/attention_runner" --kernel flash_bwd --dtype fp32 --batch 1 --heads 4 --kv-heads 4 --seq-q 64 --seq-kv 64 --head-dim 32 --causal true --warmup 2 --iters 5 --check true --csv true >> "${ATTN_BWD_OUT}"
